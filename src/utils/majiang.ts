import { Tile, TileType, VALID_TYPES } from "../types";

// 生成一副完整的麻将牌（缺一门）
function generateFullTiles(excludeType: TileType): Tile[] {
  const tiles: Tile[] = [];
...
function isTing(tiles: Tile[], excludeType: TileType): boolean {
  ...
  if (types.size !== 2 || types.has(excludeType)) return false;
...
function canFormTripletsAndSequences(tiles: Tile[]): boolean {
  ...
  if (tiles.length >= 3 && isSameTile(tiles[0], tiles[1]) && isSameTile(tiles[1], tiles[2])) {
    ...
    if (canFormTripletsAndSequences(remaining)) return true;
  }
...
// 生成听牌序列
export function generateTingTiles(): Tile[] {
...
export function getTingInfo(tiles: Tile[]): Tile[] {
  ...
  // 确保只有两种花色
  if (types.size !== 2) return tingTiles;
